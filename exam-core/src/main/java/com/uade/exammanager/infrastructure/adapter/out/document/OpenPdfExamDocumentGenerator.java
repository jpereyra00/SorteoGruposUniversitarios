package com.uade.exammanager.infrastructure.adapter.out.document;
...
/**
 * Adaptador de salida que genera el PDF del examen de un grupo con OpenPDF.
 * Consume {@link GroupExamDocument}, desacoplado de las entidades JPA.
 */
@Component
public class OpenPdfExamDocumentGenerator implements DocumentGeneratorPort {
    ...
    @Override
    public byte[] generateGroupExam(GroupExamDocument data) {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            Document document = new Document(PageSize.A4, 28, 28, 18, 24);
            PdfWriter.getInstance(document, out);
            document.open();

            addInstitutionalHeader(document, data);
            addExamIntro(document, data);
            addGroupMembersSection(document, data);
            addAssignedTopicsSection(document, data);
    ...
    private void addInstitutionalHeader(Document document, GroupExamDocument data) throws Exception {
        PdfPTable headerTable = new PdfPTable(3);
        headerTable.setWidthPercentage(100);
        headerTable.setWidths(new float[]{1f, 3f, 2f});
        headerTable.setSpacingAfter(12f);

        String dateText = data.examDate() != null ? data.examDate().format(DATE_FORMATTER) : "";

        headerTable.addCell(createLogoCell());
        headerTable.addCell(createTitleCell());
        headerTable.addCell(createHojasCell(data.pageCount()));

        PdfPCell materiaCell = createSingleLineCell("Materia:", safeText(data.subjectName()), true, 27f);
        materiaCell.setColspan(2);
        headerTable.addCell(materiaCell);
        headerTable.addCell(createSingleLineCell("Fecha:", dateText, true, 27f));

        PdfPCell nombreCell = createNombreCell(data);
        ...
        PdfPCell docentesCell = createSingleLineCell("Docentes:", safeText(data.teachers()), false, 34f);
    ...
    private PdfPCell createNombreCell(GroupExamDocument data) {
        List<GroupExamDocument.Member> members = data.members();
        ...
        if (members == null || members.isEmpty()) {
            ...
            for (int i = 0; i < members.size(); i++) {
                GroupExamDocument.Member member = members.get(i);
                nombresBuilder.append(safeText(member.fullName()))
                        .append(" (").append(safeText(member.legajo())).append(")");
                if (i < members.size() - 1) {
    ...
    private void addExamIntro(Document document, GroupExamDocument data) throws DocumentException {
        Paragraph title = new Paragraph("Metodología de Evaluación del Primer Parcial (Colaborativo)", TITLE_FONT);
        title.setSpacingAfter(6f);
        document.add(title);

        document.add(new Paragraph("Asignatura: " + safeText(data.subjectName()), BODY_BOLD_FONT));
        document.add(new Paragraph("Actividad: Evaluación Teórico-Práctica Colaborativa", BODY_BOLD_FONT));
        document.add(new Paragraph("Grupo: " + safeText(data.groupName()), BODY_BOLD_FONT));
    ...
    private void addGroupMembersSection(Document document, GroupExamDocument data) throws DocumentException {
        ...
        List<GroupExamDocument.Member> members = data.members().stream()
                .sorted(Comparator.comparing(GroupExamDocument.Member::fullName, String.CASE_INSENSITIVE_ORDER))
                .toList();
        ...
        if (members.isEmpty()) {
            ...
            for (GroupExamDocument.Member member : members) {
                String line = member.fullName() + " - Legajo: " + member.legajo();
    ...
    private void addAssignedTopicsSection(Document document, GroupExamDocument data) throws DocumentException {
        Paragraph sectionTitle = new Paragraph("Temas Asignados", SUBTITLE_FONT);
        sectionTitle.setSpacingAfter(4f);
        document.add(sectionTitle);

        List<String> topics = data.topics();
